...
class ModelLanding extends Toolkit{
		...
		public function validateRegisterForm($vars)
		{
			...
			if ( !$this->IsFieldUnique($this->users_table, 'username', $vars['username']) ) { $vars['response'] =  'Username Unavailable'; return $vars;}
			if ( !$this->IsFieldUnique($this->users_table, 'email', $vars['email']) ) { $vars['response'] = 'Email is already in use'; return $vars;}
			if ( !$this->IsFieldUnique($this->users_table, 'phone', $vars['phone']) ) { $vars['response'] = 'Email is already in use'; return $vars;}
		...
		{
			echo json_encode("INSERT INTO $this->users_table ( f_name, l_name, email, phone, username, password, confirm_code, invite_code, last_logged) values (?,?,?,?,?,?,?,?,?)");
			$success = 0;
			$this->log('insertRegisterQuery(' . $vars. ')', 'debug.html');
			$mysqli = $this->getDBC(DB_HOST, DB_USER, DB_PASS , DB_NAME);

			if($stmt = $mysqli -> prepare("INSERT INTO $this->users_table ( f_name, l_name, email, phone, username, password, confirm_code, invite_code, last_logged) values (?,?,?,?,?,?,?,?,?)"))
			{
		...
		public function CheckLoginInDB($username,$password)
		{
			...
			if($stmt = $mysqli -> prepare("SELECT CONCAT(f_name, ' ', l_name) as name, email, user_id, role, last_logged FROM $this->users_table WHERE username=? AND password=?")) {
		...
		private function IsFieldUnique($field, $value)
		{
		        ...
		        // $qry = "SELECT username FROM $this->users_table WHERE $this->Sanitize($field) ='".$this->Sanitize($value)."'";
		        // $result = mysqli_query($conn, $qry);
		        if($stmt = $mysqli -> prepare("SELECT id FROM $this->users_table WHERE $this->Sanitize($field) =?"))
		        {
		...
		private function updateUserField($field, $value)
		{
			...
			if($stmt = $mysqli->prepare("UPDATE $this->users_table SET $field= ? WHERE id=?"))
			{
